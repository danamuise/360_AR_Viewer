...
public class MapNav : MonoBehaviour 
{
    ...
    public int scaleFactor = 100;                               //Scale of the map. Default is 1:100
	...
	IEnumerator Start () {
		...
		user.position = new Vector3(0, user.position.y * 100 / scaleFactor, 0); //Centering user in the scene 
        user.localScale = new Vector3(user.localScale.x * 100 / scaleFactor, user.localScale.y * 100 / scaleFactor, user.localScale.z * 100 / scaleFactor); //rescale user according to scale factor (reference is 1:100)
		...
		// Access granted and location value could be retrieved
			...
			if(!mapDisabled){
				print("GPS location found Setting position..");
				status = "GPS location found\n Setting position ...";
			}
			else{
	...
	IEnumerator ReScale(){
        ...
        double newScale = multiplier * (10053224.4/scaleFactor) / (Mathf.Pow(2, zoom));
		...
		//3D View and Camera follows player. Set camera position
			...
			tmp.z = -((13107200 /scaleFactor / user.localScale.x) * camDist*Mathf.Cos(camAngle*Mathf.PI/180))/Mathf.Pow(2, zoom);
			tmp.y = (13107200 / scaleFactor / user.localScale.x) * camDist*Mathf.Sin(camAngle*Mathf.PI/180)/Mathf.Pow(2, zoom);
		...
		//2D View. Set camera position 
			...
			currentOrtoSize = Mathf.MoveTowards (currentOrtoSize, targetOrtoSize*0.625f, 2.5f*3276800*Time.deltaTime/Mathf.Pow(2, zoom)/scaleFactor);
			...
			if(!simGPS){
				...
				if(mapping == false){
					...
					if (Input.GetKey ("up") || Input.GetKey ("w")){
						user.transform.Translate(Vector3.forward*speed*Time.deltaTime/scaleFactor);
					}
			...
			//2D Zoom Buttons
				...
				if(GUI.RepeatButton(new Rect(2*screenX/5, 0, screenX/5, screenY/12), "zoom +")){
					if(Input.GetMouseButton(0)){
						currentOrtoSize = mycam.orthographicSize;
						currentOrtoSize = Mathf.MoveTowards (currentOrtoSize, 3*targetOrtoSize/8, (100/scaleFactor)*32768*Time.deltaTime/Mathf.Pow(2, zoom));
						mycam.orthographicSize = currentOrtoSize;
				...
				if (GUI.RepeatButton(new Rect(screenX/5, 0, screenX/5, screenY/12), "zoom -")){
					if(Input.GetMouseButton(0)){
						currentOrtoSize = mycam.orthographicSize;
						currentOrtoSize = Mathf.MoveTowards (currentOrtoSize, targetOrtoSize, (100 / scaleFactor) * 32768*Time.deltaTime/Mathf.Pow(2, zoom));
						mycam.orthographicSize = currentOrtoSize;
    ...
    {
        double _lon = (x / 20037508.34) * 180 * scaleFactor;
        double _lat = (z / 20037508.34) * 180 * scaleFactor;
    ...
    {
        double x = (_lon * 20037508.34 / 180) / scaleFactor;
        double z = (Math.Log(Math.Tan((90 + _lat) * Math.PI / 360)) / (Math.PI / 180));
        z = (z * 20037508.34 / 180) / scaleFactor;
        float y = elevation;
	...
	//change scene when clicking on a point of interest
	public void openPanoramaSFSU (){
		SceneManager.LoadScene ("Image01", LoadSceneMode.Single);
	}
	public void openPanoramaBeijing (){
		SceneManager.LoadScene ("Image01_Beijing", LoadSceneMode.Single);
	}
	public void openPanoramaChinatwon (){
		SceneManager.LoadScene ("Image01_Chinatown", LoadSceneMode.Single);
	}
	public void openAR_WashingtonGrant (){
		SceneManager.LoadScene ("AR_chinatown", LoadSceneMode.Single);
	}
}
